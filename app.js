...
const C = {
  ...
  warning: "#D4960E", danger: "#B85C42", success: "#3D6B4A",
  purple: "#7B5EA7",
  // Shadows
  shadowSm: "0 1px 3px rgba(44,31,15,0.08), 0 1px 2px rgba(44,31,15,0.06)",
  shadowMd: "0 4px 12px rgba(44,31,15,0.10), 0 2px 4px rgba(44,31,15,0.06)",
  shadowLg: "0 8px 24px rgba(44,31,15,0.12), 0 4px 8px rgba(44,31,15,0.06)",
};
...
const T = {
  nl: {
    ...
    overview:"Overzicht", seasons:"Seizoenen", care:"Verzorging", log:"Log",
...
// AI ADVISOR
// ══════════════════════════════════════════════════════════════════
const AIAdvisor = ({plant}) => {
  const [q,setQ]=useState(""), [resp,setResp]=useState(""), [loading,setLoading]=useState(false);
  const sp=SPECIES_DB[plant.species]; const season=getCurrentSeason(); const care=_optionalChain([sp, 'optionalAccess', _35 => _35.care, 'access', _36 => _36[season]]);
  const hasExpertNotes = !!(plant.expertNotes && plant.expertNotes.trim());

  const buildSystemPrompt = () => {
    const base = [
      `Je bent een specialist in plantenverzorging. De gebruiker verzorgt een ${plant.species}${sp?` (${sp.commonName})`:"" } met de naam "${plant.name}".`,
      `Huidig seizoen: ${season}.`,
      sp ? `Watergift: elke ${_optionalChain([care, 'optionalAccess', _37 => _37.water])} dagen. Luchtvochtigheid: ${_optionalChain([care, 'optionalAccess', _38 => _38.humidity])}%. Zon: ${sp.sunlight.min}–${sp.sunlight.max} uur/dag (${sp.sunlight.type}). Temperatuur: ${sp.temp.ideal}. Seizoensadvies: ${_optionalChain([care, 'optionalAccess', _39 => _39.notes])}.` : "",
      plant.health ? `Huidige gezondheid: ${plant.health}%.` : "",
      plant.notes ? `Persoonlijke notities: ${plant.notes}` : "",
    ].filter(Boolean).join(" ");

    const expertSection = hasExpertNotes
      ? `\n\nBELANGRIJK — Kwekerij/expert advies voor deze specifieke plant:\n"""\n${plant.expertNotes}\n"""\nDit advies van de kwekerij heeft PRIORITEIT boven algemene aanbevelingen. Als de gebruiker iets vraagt over water geven, licht of verzorging, baseer je antwoord dan primair op dit expert advies. Wijs op eventuele afwijkingen van de standaard zorgrichtlijnen.`
      : "";

    return base + expertSection + "\n\nAntwoord beknopt en praktisch in het Nederlands. Maximaal 3 korte alinea's.";
  };
  ...
  const ask = async () => {
    ...
    try {
      ...
      setResp(_optionalChain([data, 'access', _40 => _40.content, 'optionalAccess', _41 => _41.map, 'call', _42 => _42(b=>b.text||""), 'access', _43 => _43.join, 'call', _44 => _44("")]) || "Geen antwoord ontvangen.");
    } catch(_e) {
      setResp("Kon de AI adviseur niet bereiken. Controleer je verbinding.");
    }
    setLoading(false);
  };
  ...
  return (
    React.createElement('div', { style: {background:C.greenPale,borderRadius:12,padding:16,marginTop:16},}
      , React.createElement('div', { style: {fontWeight:700,color:C.green,marginBottom:6,fontSize:14},}, "🤖 Plant Adviseur"  )
      , hasExpertNotes && (
        React.createElement('div', { style: {fontSize:11,color:C.sand,marginBottom:10,display:"flex",alignItems:"center",gap:5,
          background:C.sand+"18",borderRadius:8,padding:"5px 8px"},}
          , React.createElement('span', null, "📋")
          , React.createElement('span', null, "Expert/kwekerij advies is meegenomen in de beantwoording"      )
        )
      )
      , React.createElement('div', { style: {display:"flex",gap:8},}
        ...
        , React.createElement(Btn, { onClick: ask, disabled: loading, small: true,}, loading?"…":"Vraag")
      )
      , resp && (
...
const PhotoGallery = ({plant, onUpdatePlant}) => {
  ...
  const savePhotos = (next) => {
    ...
    onUpdatePlant({
      ...
      photoUrl: _optionalChain([next, 'access', _45 => _45[0], 'optionalAccess', _46 => _46.url]) || null,
    });
    ...
    React.createElement('div', { style: {background:C.surface,borderRadius:14,padding:16,marginBottom:12},}
      ...
      , photos.length > 0 && (React.createElement(React.Fragment, null
        , React.createElement('div', { style: {borderRadius:12,overflow:'hidden',marginBottom:8,position:'relative',
          ...
          , React.createElement('img', { src: _optionalChain([photos, 'access', _47 => _47[activeIdx], 'optionalAccess', _48 => _48.url]), alt: "plant",
...
const CareEventsPanel = ({plant, onUpdatePlant, season}) => {
  const sp = SPECIES_DB[plant.species];
  const category = _optionalChain([sp, 'optionalAccess', _49 => _49.category]) || "";
...
const CustomCareEditor = ({plant, onUpdatePlant, season}) => {
  const sp = SPECIES_DB[plant.species];
  const care = _optionalChain([sp, 'optionalAccess', _50 => _50.care, 'access', _51 => _51[season]]) || {};
  ...
  const fields = [
    ...
    {key:"sunlight_min", label:"☀️ Zon min", unit:"uur/dag", default: _optionalChain([sp, 'optionalAccess', _52 => _52.sunlight, 'access', _53 => _53.min]) || 3, min:0, max:24,
     hint:"Minimum zonuren per dag"},
    {key:"sunlight_max", label:"☀️ Zon max", unit:"uur/dag", default: _optionalChain([sp, 'optionalAccess', _54 => _54.sunlight, 'access', _55 => _55.max]) || 8, min:0, max:24,
          ...
          React.createElement('div', { key: k, style: {marginBottom:10},}
            , React.createElement('div', { style: {display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4},}
              , React.createElement('label', { style: {fontSize:11,fontWeight:600,color:C.muted},}
                , _optionalChain([def, 'optionalAccess', _56 => _56.icon]), " " , _optionalChain([def, 'optionalAccess', _57 => _57.label, 'optionalAccess', _58 => _58.toUpperCase, 'call', _59 => _59()]), _optionalChain([def, 'optionalAccess', _60 => _60.unit])?` (${def.unit})`:""
              )
          ...
          , pKeys.map(k=>{
            ...
            return(
              React.createElement('div', { key: k, style: {background:C.surfaceAlt,borderRadius:6,padding:"3px 8px",fontSize:12,color:C.text},}
                , React.createElement('span', null, _optionalChain([def, 'optionalAccess', _61 => _61.icon]), " " )
                , React.createElement('span', { style: {fontWeight:600},}, _optionalChain([def, 'optionalAccess', _62 => _62.label]), ": " )
...
const RepottingReminder = ({plant, onUpdatePlant}) => {
  ...
  const repotInfo = _optionalChain([sp, 'optionalAccess', _63 => _63.repot]) || "Elke 2 jaar";
...
const ASPCAToxicity = ({species, staticToxicity}) => {
  ...
  const lookup = async () => {
    ...
    try {
      ...
      rows.forEach(row => {
        const scientific = _optionalChain([row, 'access', _64 => _64.querySelector, 'call', _65 => _65(".field-content"), 'optionalAccess', _66 => _66.textContent, 'optionalAccess', _67 => _67.trim, 'call', _68 => _68()]) || "";
        const toxicStatus = _optionalChain([row, 'access', _69 => _69.querySelector, 'call', _70 => _70(".views-field-field-plant-tox-status"), 'optionalAccess', _71 => _71.textContent, 'optionalAccess', _72 => _72.trim, 'call', _73 => _73()]) || "";
        const animals = _optionalChain([row, 'access', _74 => _74.querySelector, 'call', _75 => _75(".views-field-field-plant-species-animals"), 'optionalAccess', _76 => _76.textContent, 'optionalAccess', _77 => _77.trim, 'call', _78 => _78()]) || "";
        const symptoms = _optionalChain([row, 'access', _79 => _79.querySelector, 'call', _80 => _80(".views-field-field-plant-species-clinical-signs"), 'optionalAccess', _81 => _81.textContent, 'optionalAccess', _82 => _82.trim, 'call', _83 => _83()]) || "";
        const name = _optionalChain([row, 'access', _84 => _84.querySelector, 'call', _85 => _85("h3, .views-field-title"), 'optionalAccess', _86 => _86.textContent, 'optionalAccess', _87 => _87.trim, 'call', _88 => _88()]) || "";
      ...
      if (found) {
        ...
        const pageText = _optionalChain([doc, 'access', _89 => _89.body, 'optionalAccess', _90 => _90.innerText]) || html;
...
const LichtMeter = ({plant, sunlight, onSaveReading, onClose}) => {
  ...
  // Cleanup on unmount
  useEffect(() => () => {
    _optionalChain([streamRef, 'access', _91 => _91.current, 'optionalAccess', _92 => _92.getTracks, 'call', _93 => _93(), 'access', _94 => _94.forEach, 'call', _95 => _95(t => t.stop())]);
    cancelAnimationFrame(rafRef.current);
    _optionalChain([sensorRef, 'access', _96 => _96.current, 'optionalAccess', _97 => _97.stop, 'optionalCall', _98 => _98()]);
  }, []);
  ...
  const saveReading = () => {
    if (lux === null) return;
    _optionalChain([onSaveReading, 'optionalCall', _99 => _99(lux, _optionalChain([zone, 'optionalAccess', _100 => _100.label]) || "")]);
    onClose();
  };
      ...
      , React.createElement('div', { style: {padding:"14px 16px",display:"flex",alignItems:"center",gap:12,
        ...
        , React.createElement('div', { style: {flex:1},}
          ...
          , plant && React.createElement('div', { style: {fontSize:11,color:"rgba(255,255,255,0.5)"},}, "voor "
             , plant.name, " · "  , _optionalChain([sunlight, 'optionalAccess', _101 => _101.min]), "–", _optionalChain([sunlight, 'optionalAccess', _102 => _102.max]), "u/dag vereist"
          )
        )
        , method === "sensor" && (
        ...
        React.createElement('div', { style: {flex:1,display:"flex",flexDirection:"column",overflow:"hidden"},}
          ...
          , React.createElement('div', { style: {flex:1,overflowY:"auto",padding:"20px 20px 0",textAlign:"center"},}
            ...
            , React.createElement('div', { style: {fontWeight:800,fontSize:18,color:"#fff",marginBottom:10},}, "Licht meten voor "
                 , _optionalChain([plant, 'optionalAccess', _103 => _103.name]) || "je plant"
            )
    ...
    React.createElement('div', { style: {position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:400,
      ...
      , React.createElement('div', { style: {background:C.surface,borderRadius:24,padding:24,width:"100%",maxWidth:360,
        ...
        , React.createElement('div', { id: `qr-card-${plant.id}`, style: {background:"#fff",borderRadius:16,padding:16,
          ...
          , _optionalChain([sp, 'optionalAccess', _104 => _104.toxicity]) && !/non[\-\s]toxic/i.test(sp.toxicity) && (
...
const QRScanner = ({plants, onFound, onClose}) => {
  ...
  useEffect(() => {
    let animFrame;
    _optionalChain([navigator, 'access', _105 => _105.mediaDevices, 'optionalAccess', _106 => _106.getUserMedia, 'call', _107 => _107({video:{facingMode:"environment"}})
, 'access', _108 => _108.then, 'call', _109 => _109(stream => {
        streamRef.current = stream;
        if (videoRef.current) {
          videoRef.current.srcObject = stream;
        ...
        const scan = () => {
          ...
          if (video && canvas && video.readyState === video.HAVE_ENOUGH_DATA) {
            ...
            // Simple pattern match: look for plant ID in decoded data
            // Without jsQR we match by scanning the frame for known plant IDs
            // This is a simplified approach — for production use jsQR via CDN when online
            try {
              // Try to read any text-encoded QR from the image
              // We look for our plant ID pattern in the raw pixels
              const data = imageData.data;
              // Placeholder: in HTTP local context, show manual entry fallback
            } catch(_e) {}
          }
...
, 'access', _110 => _110.catch, 'call', _111 => _111(() => setError("Geen cameratoegang. Controleer je browserinstellingen."))]);
    return () => {
      cancelAnimationFrame(animFrame);
      _optionalChain([streamRef, 'access', _112 => _112.current, 'optionalAccess', _113 => _113.getTracks, 'call', _114 => _114(), 'access', _115 => _115.forEach, 'call', _116 => _116(t=>t.stop())]);
    };
  }, []);

  // Manual ID entry fallback (works offline without jsQR)
  const [manualId, setManualId] = useState("");
  const handleManual = () => {
    const plant = plants.find(p=>p.id===manualId.trim());
    if (plant) onFound(plant.id);
    else setError(`Geen plant gevonden met ID: ${manualId}`);
  };

  return (
    React.createElement('div', { style: {position:"fixed",inset:0,background:"#000",zIndex:400,
      display:"flex",flexDirection:"column"},}
      , React.createElement('div', { style: {padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"},}
        , React.createElement('div', { style: {fontWeight:700,fontSize:16,color:"#fff"},}, "📱 QR Scanner"  )
        , React.createElement('button', { onClick: onClose, style: {background:"none",border:"none",color:"#fff",
      ...
      , error ? (
        React.createElement('div', { style: {flex:1,display:"flex",flexDirection:"column",alignItems:"center",
          ...
          , React.createElement('div', { style: {fontSize:48,marginBottom:16},}, "📷")
          , React.createElement('div', { style: {marginBottom:20,fontSize:14,lineHeight:1.6,color:"rgba(255,255,255,0.7)"},}, error)
          , React.createElement('div', { style: {width:"100%",maxWidth:320},}
            , React.createElement('div', { style: {fontSize:13,color:"rgba(255,255,255,0.5)",marginBottom:8},}, "Of voer plant-ID handmatig in:"    )
            , React.createElement('div', { style: {display:"flex",gap:8},}
        ...
        React.createElement('div', { style: {flex:1,position:"relative"},}
          ...
          , React.createElement('div', { style: {position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"},}
            , React.createElement('div', { style: {width:220,height:220,border:"2px solid rgba(255,255,255,0.8)",borderRadius:16,
              boxShadow:"0 0 0 2000px rgba(0,0,0,0.5)"},}
              , React.createElement('div', { style: {position:"absolute",top:-2,left:-2,width:30,height:30,
                borderTop:"3px solid #4AFF91",borderLeft:"3px solid #4AFF91",borderRadius:"14px 0 0 0"},})
              , React.createElement('div', { style: {position:"absolute",top:-2,right:-2,width:30,height:30,
                borderTop:"3px solid #4AFF91",borderRight:"3px solid #4AFF91",borderRadius:"0 14px 0 0"},})
              , React.createElement('div', { style: {position:"absolute",bottom:-2,left:-2,width:30,height:30,
                borderBottom:"3px solid #4AFF91",borderLeft:"3px solid #4AFF91",borderRadius:"0 0 0 14px"},})
              , React.createElement('div', { style: {position:"absolute",bottom:-2,right:-2,width:30,height:30,
                borderBottom:"3px solid #4AFF91",borderRight:"3px solid #4AFF91",borderRadius:"0 0 14px 0"},})
            )
          )
          , React.createElement('div', { style: {position:"absolute",bottom:20,left:0,right:0,textAlign:"center",
            color:"rgba(255,255,255,0.7)",fontSize:13},}, "Houd de QR-code in het kader"
...
const PDFExport = ({plants, logs, onClose}) => {
  ...
  const printReport = () => {
    ...
    const rows = exportPlants.map(p => {
      ...
      const lastWater = plantLogs.find(l=>_optionalChain([l, 'access', _117 => _117.parameters, 'optionalAccess', _118 => _118.watering]));
      const toxic = _optionalChain([sp, 'optionalAccess', _119 => _119.toxicity]) && !/non[\-\s]toxic/i.test(sp.toxicity);
      return `
        <div class="plant-card">
          ...
          <div class="info-grid">
            ...
            ${sp?`<div class="info-item"><span class="label">Waterfrequentie</span><span>${_optionalChain([sp, 'access', _120 => _120.care, 'access', _121 => _121.summer, 'optionalAccess', _122 => _122.water])} dagen (zomer)</span></div>`:""}
          ...
          ${_optionalChain([p, 'access', _123 => _123.soilMix, 'optionalAccess', _124 => _124.length])?`<div class="soil-mix"><strong>Grondmengsel:</strong> ${p.soilMix.map(m=>`${m.pct}% ${m.name}`).join(", ")}</div>`:""}
        ...
        , React.createElement('div', { style: {overflowY:"auto",flex:1,padding:"14px 20px"},}
          ...
          , plants.map(p=>(
            React.createElement('button', { key: p.id, onClick: ()=>toggle(p.id),
              ...
              , React.createElement(ToxicityBadge, { toxicity: _optionalChain([SPECIES_DB, 'access', _125 => _125[p.species], 'optionalAccess', _126 => _126.toxicity]), small: true,})
            )
          ))
        )
        , React.createElement('div', { style: {padding:"12px 20px",borderTop:`1px solid ${C.border}`,display:"flex",gap:10},}
...
const PlantDetail = ({plant,logs,sections,onAddSection,onBack,onUpdatePlant,onAddLog,onDeleteLog,onUpdateLog,onDelete,t}) => {
  ...
  useEffect(()=>{
    ...
    const check=()=>{
      ...
      if(e && _optionalChain([e, 'access', _127 => _127.care, 'optionalAccess', _128 => _128.spring, 'optionalAccess', _129 => _129.water]) && (!dbEntry || !_optionalChain([dbEntry, 'access', _130 => _130.care, 'optionalAccess', _131 => _131.spring, 'optionalAccess', _132 => _132.water]))) {
        setDbEntry(e);
      }
    };
    check();
  ...
  const userPhotoUrl = _optionalChain([(plant.photos || []), 'access', _133 => _133[0], 'optionalAccess', _134 => _134.url]) || null;
  ...
  useEffect(() => {
    ...
    const up = _optionalChain([(plant.photos || []), 'access', _135 => _135[0], 'optionalAccess', _136 => _136.url]) || null;
  ...
  const emoji=_optionalChain([sp, 'optionalAccess', _137 => _137.image])||"🌿";
  ...
  return(
    React.createElement('div', { style: {minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif"},}
      ...
      , waterPaused ? (
        React.createElement('div', { style: {background:C.warning,padding:"10px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"},}
          , React.createElement('span', { style: {color:"#fff",fontWeight:700,fontSize:14},}, "⚡ "
             , _optionalChain([pausedEvt, 'optionalAccess', _138 => _138.icon]), " Gepauzeerd — "   , _optionalChain([pausedEvt, 'optionalAccess', _139 => _139.label])
          )
          , React.createElement('span', { style: {color:"rgba(255,255,255,0.8)",fontSize:12},}, "nog "
             , Math.ceil((new Date(_optionalChain([pausedEvt, 'optionalAccess', _140 => _140.endDate]))-new Date())/86400000), " dagen"
          )
        )
      ) : days!==null&&(
        React.createElement('div', { style: {background:urgColor(days),padding:"10px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8},}
          , React.createElement('span', { style: {color:"#fff",fontWeight:700,fontSize:13,flex:1},}
            , days<=0?(_optionalChain([t, 'optionalAccess', _141 => _141.waterToday])||"💧 Water vandaag!"):days===1?(_optionalChain([t, 'optionalAccess', _142 => _142.waterTomorrow])||"💧 Water morgen"):`${_optionalChain([t, 'optionalAccess', _143 => _143.waterIn])||"💧 Water over"} ${days} ${days===1?(_optionalChain([t, 'optionalAccess', _144 => _144.day])||"dag"):(_optionalChain([t, 'optionalAccess', _145 => _145.days])||"dagen")}`
          )
          ...
          , React.createElement('button', { onClick: handleWatered,
            ...
            , _optionalChain([t, 'optionalAccess', _146 => _146.markWatered])||"✓ Water gegeven"
          )
        )
      )
      ...
      , React.createElement('div', { style: {display:"flex",borderBottom:`2px solid ${C.border}`,background:C.surface},}
        , [
          {id:"overview", label:_optionalChain([t, 'optionalAccess', _147 => _147.overview])||"Overzicht",  icon:"🏠"},
          {id:"seasons",  label:_optionalChain([t, 'optionalAccess', _148 => _148.seasons])||"Seizoenen",   icon:"🗓"},
          {id:"care",     label:_optionalChain([t, 'optionalAccess', _149 => _149.care])||"Verzorging",     icon:"🌿"},
          {id:"log",      label:`${_optionalChain([t, 'optionalAccess', _150 => _150.log])||"Log"} (${plantLogs.length})`, icon:"📝"},
        ].map(tb=>(
      ...
      , React.createElement('div', { style: {padding:16,maxWidth:600,margin:"0 auto",paddingBottom:40},}
        ...
        , tab==="overview"&&(
          React.createElement(React.Fragment, null
            ...
            , React.createElement(CollapsibleCard, {
              ...
              badge: _optionalChain([plant, 'access', _151 => _151.expertNotes, 'optionalAccess', _152 => _152.trim, 'call', _153 => _153()])?"Expert advies":null,
              urgent: false,}
              , React.createElement('div', { style: {paddingTop:12},}
            ...
            , React.createElement(CollapsibleCard, {
              ...
              , sp ? (
                React.createElement('div', { style: {paddingTop:12},}
                  ...
                  , _optionalChain([care, 'access', _154 => _154.expertBullets, 'optionalAccess', _155 => _155.length]) > 0 && (
            ...
            , React.createElement(CollapsibleCard, { icon: "🤖", title: "AI Plant Advisor"  ,
              defaultOpen: false,
              badge: _optionalChain([care, 'access', _156 => _156.expertBullets, 'optionalAccess', _157 => _157.length])>0?`📋 ${care.expertBullets.length} tips`:null,}
              , React.createElement('div', { style: {paddingTop:12},}
                , React.createElement(AIAdvisor, { plant: plant,})
              )
            )
        ...
        , tab==="seasons"&&(React.createElement(React.Fragment, null
          ...
          , SEASONS.map(s=>{
            ...
            const spCare = _optionalChain([sp, 'optionalAccess', _158 => _158.care, 'access', _159 => _159[s]]) || {water:7, humidity:50, notes:""};
            const custom = plant.customCare || {};
        ...
        , tab==="log"&&(React.createElement(React.Fragment, null
          ...
          , (() => {
            const healthPoints = plantLogs
              .filter(l => _optionalChain([l, 'access', _160 => _160.parameters, 'optionalAccess', _161 => _161.health_score]) !== undefined)
              .map(l => ({date:l.timestamp.split("T")[0], val:l.parameters.health_score}))
              .sort((a,b)=>a.date.localeCompare(b.date));
            const heightPoints = plantLogs
              .filter(l => _optionalChain([l, 'access', _162 => _162.parameters, 'optionalAccess', _163 => _163.height]) !== undefined)
          ...
          , (() => {
            const waterLogs = plantLogs.filter(l => _optionalChain([l, 'access', _164 => _164.parameters, 'optionalAccess', _165 => _165.watering]) !== undefined);
  ...
  try {
    ...
    const results = (Array.isArray(data) ? data : data.results || [])
      ...
      .map(s => ({
        ...
        commonName: _optionalChain([s, 'access', _166 => _166.vernacularNames, 'optionalAccess', _167 => _167[0], 'optionalAccess', _168 => _168.vernacularName]) || s.canonicalName,
    ...
    return (data2.results || [])
      ...
      .map(s => ({
        scientificName: s.canonicalName,
        commonName: _optionalChain([s, 'access', _169 => _169.vernacularNames, 'optionalAccess', _170 => _170[0], 'optionalAccess', _171 => _171.vernacularName]) || s.canonicalName,
        family: s.family || "",
        fromDB: !!SPECIES_DB[s.canonicalName],
      }));
  } catch (e) {
...
const fetchWikiPhoto = async (speciesName) => {
  try {
    ...
    const title = _optionalChain([sd, 'optionalAccess', _172 => _172.query, 'optionalAccess', _173 => _173.search, 'optionalAccess', _174 => _174[0], 'optionalAccess', _175 => _175.title]);
    ...
    const pages = _optionalChain([id, 'optionalAccess', _176 => _176.query, 'optionalAccess', _177 => _177.pages]);
    const page = pages ? Object.values(pages)[0] : null;
    return _optionalChain([page, 'optionalAccess', _178 => _178.thumbnail, 'optionalAccess', _179 => _179.source]) || null;
...
const fetchAndStoreSpeciesData = async (scientificName, commonName, family) => {
  ...
  if (existing && _optionalChain([existing, 'access', _180 => _180.care, 'optionalAccess', _181 => _181.spring, 'optionalAccess', _182 => _182.water])) return existing;
...
{"commonName":"string","family":"string","category":"Cacti|Succulents|Aroids|Ferns|Palms|Orchids|Bromeliads|Tropical Foliage|Herbs|Carnivorous|Climbing / Trailing|Flowering|Indoor Trees|Other","origin":"string","image":"emoji","careLevel":"Easy|Medium|Hard","description":"2-3 sentences","care":{"spring":{"water":10,"humidity":40,"notes":"spring advice"},"summer":{"water":7,"humidity":45,"notes":"summer advice"},"autumn":{"water":14,"humidity":35,"notes":"autumn advice"},"winter":{"water":21,"humidity":30,"notes":"winter advice"}},"sunlight":{"min":4,"max":8,"type":"light description","uv":"Low|Medium|High|Very High"},"temp":{"min":5,"max":35,"ideal":"15-28°C"},"soil":"soil description","pot":"pot description","fertilize":"fertilizing info","repot":"repotting info","toxicity":"toxicity info","propagation":"propagation method","tips":["tip 1","tip 2","tip 3"]}`;
  ...
  try {
    ...
    let text = (_optionalChain([data, 'access', _183 => _183.content, 'optionalAccess', _184 => _184.map, 'call', _185 => _185(b => b.text || ""), 'access', _186 => _186.join, 'call', _187 => _187("")]) || "").trim();
    ...
    if (!_optionalChain([entry, 'access', _188 => _188.care, 'optionalAccess', _189 => _189.spring, 'optionalAccess', _190 => _190.water])) { console.warn("Incomplete entry:", entry); return null; }
...
const reverseGeocode = async (lat, lon) => {
  try {
    ...
    return {
      city:    _optionalChain([d, 'access', _191 => _191.address, 'optionalAccess', _192 => _192.city]) || _optionalChain([d, 'access', _193 => _193.address, 'optionalAccess', _194 => _194.town]) || _optionalChain([d, 'access', _195 => _195.address, 'optionalAccess', _196 => _196.village]) || _optionalChain([d, 'access', _197 => _197.address, 'optionalAccess', _198 => _198.county]) || "",
      country: _optionalChain([d, 'access', _199 => _199.address, 'optionalAccess', _200 => _200.country]) || "",
      countryCode: _optionalChain([d, 'access', _201 => _201.address, 'optionalAccess', _202 => _202.country_code, 'optionalAccess', _203 => _203.toUpperCase, 'call', _204 => _204()]) || "",
...
const fetchINaturalist = async (speciesName) => {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    const taxon = _optionalChain([d, 'access', _205 => _205.results, 'optionalAccess', _206 => _206[0]]);
    ...
    return {
      ...
      status:       _optionalChain([taxon, 'access', _207 => _207.conservation_status, 'optionalAccess', _208 => _208.status_name]) || null,
    };
  } catch(_e) { return null; }
};
...
const fetchLocationAwareCareData = async (scientificName, commonName, family, onProgress) => {
  ...
  entry._sources = {
    weather:     !!weather,
    wikipedia:   !!_optionalChain([wiki, 'optionalAccess', _209 => _209.extract]),
    inaturalist: !!inat,
...
const buildCareFromSources = (scientificName, commonName, family, wiki, inat, location, weather) => {
  ...
  const name = (scientificName + " " + (commonName||"") + " " + (family||"") + " " + (_optionalChain([wiki, 'optionalAccess', _210 => _210.extract])||"")).toLowerCase();
  ...
  const localHumidity = _optionalChain([weather, 'optionalAccess', _211 => _211.current, 'optionalAccess', _212 => _212.humidity]) || 60;
  const localTemp = _optionalChain([weather, 'optionalAccess', _213 => _213.current, 'optionalAccess', _214 => _214.temp]) || 15;
  ...
  const extract = _optionalChain([wiki, 'optionalAccess', _215 => _215.extract]) || "";
  ...
  return {
    ...
    category,
    origin: extractOrigin(_optionalChain([wiki, 'optionalAccess', _216 => _216.extract]) || "", inat),
    image: emoji,
    ...
    tips: [
      ...
      inat ? `${_optionalChain([inat, 'access', _217 => _217.observations, 'optionalAccess', _218 => _218.toLocaleString, 'call', _219 => _219()]) || "Duizenden"} waarnemingen wereldwijd op iNaturalist.` : "Controleer regelmatig op plagen.",
    ],
...
Wikipedia says: "${(_optionalChain([wiki, 'optionalAccess', _220 => _220.extract]) || "").slice(0, 400)}"
...
{"care":{"spring":{"water":N,"humidity":N,"notes":"string"},"summer":{"water":N,"humidity":N,"notes":"string"},"autumn":{"water":N,"humidity":N,"notes":"string"},"winter":{"water":N,"humidity":N,"notes":"string"}},"tips":["tip1","tip2","tip3"],"locationNote":"1 sentence about growing in ${locCity}"}`;
  ...
  let text = (_optionalChain([data, 'access', _221 => _221.content, 'optionalAccess', _222 => _222.map, 'call', _223 => _223(b => b.text||""), 'access', _224 => _224.join, 'call', _225 => _225("")])||"").trim();
  text = text.replace(/^```(?:json)?\s*/i,"").replace(/\s*```\s*$/i,"").trim();
  const s = text.indexOf("{"), e = text.lastIndexOf("}");
  if (s === -1) return null;
  const patch = JSON.parse(text.slice(s, e+1));
  // Only apply valid patches
  if (_optionalChain([patch, 'access', _226 => _226.care, 'optionalAccess', _227 => _227.spring, 'optionalAccess', _228 => _228.water])) Object.assign(baseEntry.care, patch.care);
  if (_optionalChain([patch, 'access', _229 => _229.tips, 'optionalAccess', _230 => _230.length])) baseEntry.tips = patch.tips;
...
const RefreshCareData = ({plant, onUpdatePlant}) => {
  const sp = SPECIES_DB[plant.species];
  const sources = _optionalChain([sp, 'optionalAccess', _231 => _231._sources]);
  ...
  const run = async () => {
    ...
    if (SPECIES_DB[plant.species] && !_optionalChain([SPECIES_DB, 'access', _232 => _232[plant.species], 'optionalAccess', _233 => _233.care, 'optionalAccess', _234 => _234.spring, 'optionalAccess', _235 => _235.water])) {
    ...
    const out = await fetchLocationAwareCareData(
      plant.species,
      _optionalChain([existingEntry, 'optionalAccess', _236 => _236.commonName]) || plant.species,
      _optionalChain([existingEntry, 'optionalAccess', _237 => _237.family]) || "",
    ...
    if (out) {
      ...
      const season = getCurrentSeason();
      const newInterval = _optionalChain([out, 'access', _238 => _238.entry, 'optionalAccess', _239 => _239.care, 'optionalAccess', _240 => _240[season], 'optionalAccess', _241 => _241.water]) || 7;
  ...
  return (
    React.createElement('div', { style: {background:C.surface,borderRadius:14,padding:16,marginBottom:12,
      ...
      , _optionalChain([sources, 'optionalAccess', _242 => _242.locationNote]) && (
  ...
  const search = useCallback(async (q) => {
    ...
    const localMatches = Object.entries(SPECIES_DB)
      .filter(([k, v]) =>
        ...
        _optionalChain([v, 'access', _243 => _243.commonName, 'optionalAccess', _244 => _244.toLowerCase, 'call', _245 => _245(), 'access', _246 => _246.includes, 'call', _247 => _247(ql)]) ||
        _optionalChain([v, 'access', _248 => _248.family, 'optionalAccess', _249 => _249.toLowerCase, 'call', _250 => _250(), 'access', _251 => _251.includes, 'call', _252 => _252(ql)]) ||
        _optionalChain([v, 'access', _253 => _253.category, 'optionalAccess', _254 => _254.toLowerCase, 'call', _255 => _255(), 'access', _256 => _256.includes, 'call', _257 => _257(ql)])
      )
...
const AddPlantModal = ({onAdd, sections, onClose}) => {
  ...
  useEffect(() => {
    ...
    if (!name) setName(_optionalChain([selected, 'access', _258 => _258.commonName, 'optionalAccess', _259 => _259.split, 'call', _260 => _260(" "), 'access', _261 => _261[0]]) || "");
  }, [_optionalChain([selected, 'optionalAccess', _262 => _262.scientificName])]);
  ...
  const handleAdd = async () => {
    ...
    if (!SPECIES_DB[selected.scientificName]) {
      ...
      try {
        ...
        entry = _optionalChain([result, 'optionalAccess', _263 => _263.entry]) || null;
      } catch(_e) {}
      if (!entry) {
        ...
        , React.createElement('div', { style: {overflowY:"auto",flex:1,padding:"0 20px"},}
          ...
          , results.length>0&&!selected&&results.map(r=>(
            React.createElement('div', { key: r.scientificName, onClick: ()=>setSelected(r),
              ...
              , React.createElement('div', { style: {fontSize:28,width:40,textAlign:"center",flexShrink:0},}, _optionalChain([SPECIES_DB, 'access', _264 => _264[r.scientificName], 'optionalAccess', _265 => _265.image])||"🌿")
              , React.createElement('div', { style: {flex:1,minWidth:0},}
            ...
            React.createElement('div', null
              ...
              , React.createElement('div', { style: {display:"flex",gap:14,marginBottom:16,alignItems:"flex-start"},}
                , React.createElement('div', { style: {width:110,height:110,borderRadius:14,overflow:"hidden",background:C.surfaceAlt,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,position:"relative"},}
                  , loadingPhoto?(React.createElement('div', { style: {textAlign:"center"},}, React.createElement('div', { style: {fontSize:36},}, _optionalChain([sp, 'optionalAccess', _266 => _266.image])||"🌿"), React.createElement('div', { style: {fontSize:9,color:C.muted,marginTop:4},}, "Laden…")))
                  ...
                  :(React.createElement('div', { style: {textAlign:"center"},}, React.createElement('div', { style: {fontSize:40},}, _optionalChain([sp, 'optionalAccess', _267 => _267.image])||"🌿"), React.createElement('div', { style: {fontSize:9,color:C.muted,marginTop:2},}, "Geen foto" )))
                )
                , React.createElement('div', { style: {flex:1},}
        ...
        if (daysLeft <= 0) {
          ...
          if (settings.missedWateringAlert) {
            sendNotification(
              ...
              `${name} (${_optionalChain([sp, 'optionalAccess', _268 => _268.commonName]) || plant.species}) had ${Math.abs(daysLeft)} dag${Math.abs(daysLeft)!==1?"en":""} geleden water moeten krijgen.`,
              `water_${plant.id}`
            );
...
const PlantCard = ({plant, logs, onSelect, sections, onQuickWater, isSelected, bulkMode}) => {
  ...
  const emoji = _optionalChain([sp, 'optionalAccess', _269 => _269.image]) || "🌿";
  ...
  return (
    React.createElement('div', { className: "pca-card", style: {background:isSelected?C.greenPale:C.surface,borderRadius:18,marginBottom:10,
      ...
      , React.createElement('div', { onClick: ()=>onSelect(plant),
        ...
        , React.createElement('div', { style: {flex:1,minWidth:0},}
          , React.createElement('div', { style: {display:"flex",alignItems:"center",gap:6},}
            ...
            , _optionalChain([SPECIES_DB, 'access', _270 => _270[plant.species], 'optionalAccess', _271 => _271.toxicity]) && !/non[\-\s]toxic/i.test(SPECIES_DB[plant.species].toxicity) && (
          ...
          , React.createElement('div', { style: {fontSize:11,color:C.muted,marginTop:2},}, "📍 " , plant.location, " · 📝 "   , logCount, " logs" )
        )
          ...
          , waterPaused ? (
            React.createElement('div', { style: {background:C.warning+"18",borderRadius:8,padding:"4px 6px"},}
              ...
              , React.createElement('div', { style: {fontSize:13,color:C.warning,fontWeight:800,lineHeight:1.2},}
                , Math.ceil((new Date(_optionalChain([pausedEvent, 'optionalAccess', _272 => _272.endDate]))-new Date())/86400000), "d"
              )
            )
          ) : snoozed ? (
      ...
      , !waterPaused && !bulkMode && d !== null && d <= 2 && (
...
const SettingsScreen = ({
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        onImport(data);
        setImportMsg({type:"ok", text:`✅ ${data.plants.length} planten en ${_optionalChain([data, 'access', _273 => _273.logs, 'optionalAccess', _274 => _274.length])||0} logs geïmporteerd.`});
      } catch(err) {
  ...
  return (
    React.createElement('div', { style: {position:"fixed",inset:0,background:C.bg,zIndex:200,display:"flex",flexDirection:"column",fontFamily:"system-ui,sans-serif"},}
      /* Header */
      , React.createElement('div', { style: {background:`linear-gradient(160deg, ${C.soil} 0%, ${C.soilLight} 100%)`,
        ...
        , React.createElement('div', { style: {fontSize:24,fontWeight:900,color:"#F5F1EB",marginBottom:0,
          paddingBottom:14,letterSpacing:-0.3},}, "⚙️ Instellingen" )
        /* Tab bar */
      ...
      /* Content */
      , React.createElement('div', { style: {flex:1,overflowY:"auto",padding:"16px",maxWidth:600,width:"100%",margin:"0 auto",boxSizing:"border-box"},}
        ...
        , tab === "general" && (
          React.createElement('div', null
            ...
            , React.createElement('div', { style: {background:C.surface,borderRadius:14,padding:16,marginBottom:12},}
              ...
              , [
                ["Planten met foto", plants.filter(p=>p.photoUrl||_optionalChain([p, 'access', _275 => _275.photos, 'optionalAccess', _276 => _276.length])).length],
                ["Planten met zorgdata", plants.filter(p=>_optionalChain([SPECIES_DB, 'access', _277 => _277[p.species], 'optionalAccess', _278 => _278.care])).length],
                ...
                ["Logs deze maand", logs.filter(l=>_optionalChain([l, 'access', _279 => _279.timestamp, 'optionalAccess', _280 => _280.startsWith, 'call', _281 => _281(new Date().toISOString().slice(0,7))])).length],
              ].map(([label,val])=>(
            ...
            , React.createElement('div', { style: {background:C.surface,borderRadius:14,overflow:"hidden",marginBottom:12},}
              , [
                {label:"📂 Afdelingen beheren", action:()=>setTab("sections")},
                {label:"🔔 Meldingen instellen", action:()=>setTab("notif")},
                {label:"💾 Data exporteren / importeren", action:()=>setTab("data")},
              ].map(({label,action})=>(
                React.createElement('button', { key: label, onClick: action,
                  style: {width:"100%",padding:"14px 16px",border:"none",borderBottom:`1px solid ${C.border}`,
                    background:"none",cursor:"pointer",textAlign:"left",fontSize:14,color:C.soil,
                    fontFamily:"inherit",display:"flex",justifyContent:"space-between",alignItems:"center"},}
                  , label, " " , React.createElement('span', { style: {color:C.muted},}, "›")
                )
              ))
            )
          )
        ...
        , tab === "data" && (
          React.createElement('div', null
            ...
            , React.createElement('div', { style: {background:C.surface,borderRadius:14,padding:14,marginBottom:12,
              border:`1px solid ${C.border}`},}
              , React.createElement('div', { style: {fontWeight:700,color:C.soil,marginBottom:4,fontSize:14},}, "📄 Rapport exporteren"  )
              , React.createElement('div', { style: {fontSize:13,color:C.muted,marginBottom:10,lineHeight:1.5},}, "Genereer een overzicht van al je planten als PDF — handig voor een verzorger of als archief."

              )
              , React.createElement(PDFExportButton, { plants: plants, logs: logs,})
            )
            , React.createElement('div', { style: {background:C.greenPale,borderRadius:12,padding:14,marginBottom:12},}
              , React.createElement('div', { style: {fontWeight:700,color:C.green,marginBottom:6,fontSize:14},}, "📤 Exporteren" )
              , React.createElement('div', { style: {fontSize:13,color:C.soil,marginBottom:10,lineHeight:1.5},}, "Download al je data als JSON-bestand. Gebruik dit om te migreren naar een nieuwe URL, nieuw apparaat, of als backup."
              ...
              , React.createElement('button', { onClick: handleExport,
                style: {width:"100%",padding:"11px",border:"none",borderRadius:10,
                  background:C.green,color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:14,fontWeight:700},}, "📤 Download backup"

              )
            )

            , React.createElement('div', { style: {background:C.surface,borderRadius:12,padding:14,border:`1px solid ${C.border}`},}
              , React.createElement('div', { style: {fontWeight:700,color:C.soil,marginBottom:6,fontSize:14},}, "📥 Importeren" )
              , React.createElement('div', { style: {fontSize:13,color:C.muted,marginBottom:10,lineHeight:1.5},}, "Laad een backup-bestand. Dit "
                    , React.createElement('strong', null, "overschrijft"), " alle huidige data."
              )
              , importMsg && (
                React.createElement('div', { style: {background:importMsg.type==="ok"?C.greenPale:C.terracottaLight,
                  borderRadius:8,padding:10,fontSize:13,
                  color:importMsg.type==="ok"?C.green:C.terracotta,marginBottom:10,lineHeight:1.5},}
                  , importMsg.text
                )
              )
              , React.createElement('label', null
...
const Dashboard = ({plants, logs, sections, estates, activeEstate, onSelectPlant, onAddPlant, onDeletePlant, onOpenSettings, onQuickWater, t, bulkMode, selectedPlants, onToggleBulk, onBulkWater, onBulkMove, allSections, onOpenScanner}) => {
  ...
  const displayed = useMemo(() => {
    ...
    if (filterMode === "category" && filterCat !== "Alle")
      list = list.filter(p => _optionalChain([SPECIES_DB, 'access', _282 => _282[p.species], 'optionalAccess', _283 => _283.category]) === filterCat);
  ...
  const usedCats = [...new Set(plants.map(p => _optionalChain([SPECIES_DB, 'access', _284 => _284[p.species], 'optionalAccess', _285 => _285.category])).filter(Boolean))].sort();
    ...
    React.createElement('div', { style: {minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",paddingBottom:80},}
      ...
      , React.createElement('div', { style: {background:`linear-gradient(160deg, ${C.soil} 0%, ${C.soilLight} 60%, #3A2E1A 100%)`,
        ...
        , React.createElement('div', { style: {display:"flex",justifyContent:"space-between",alignItems:"flex-start",position:"relative"},}
          , React.createElement('div', null
            , React.createElement('div', { style: {fontSize:10,color:C.sand,letterSpacing:3,textTransform:"uppercase",
              ...
              , React.createElement('span', null, (_optionalChain([t, 'optionalAccess', _286 => _286.seasons])||season).charAt(0).toUpperCase()+(_optionalChain([t, 'optionalAccess', _287 => _287.seasons])||season).slice(1), " " , new Date().getFullYear())
            )
            , React.createElement('div', { style: {fontSize:10,color:"rgba(255,255,255,0.5)",marginBottom:2,
              display:"flex",alignItems:"center",gap:4},}
              , React.createElement('span', null, _optionalChain([activeEstate, 'optionalAccess', _288 => _288.icon])||"🌿")
              , React.createElement('span', { style: {letterSpacing:1,textTransform:"uppercase",fontWeight:600},}
                , _optionalChain([activeEstate, 'optionalAccess', _289 => _289.name])||"Mijn Tuin"
              )
            )
            ...
            , React.createElement('div', { style: {fontSize:12,color:C.greenLight,marginTop:4,fontWeight:500,
              ...
              , React.createElement('button', { onClick: onOpenSettings,
                ...
                , _optionalChain([t, 'optionalAccess', _290 => _290.switchLocation])||"+ Locatie wisselen"
              )
            )
          )
        ...
        , React.createElement('div', { style: {display:"flex",gap:8,marginTop:16,position:"relative"},}
          , [
            {label:_optionalChain([t, 'optionalAccess', _291 => _291.waterNow])||"Water nu",val:urgent.length,color:urgent.length?"#FF8A80":C.greenLight,
             bg:urgent.length?"rgba(184,92,66,0.25)":"rgba(255,255,255,0.08)",
             icon:"💧"},
            {label:_optionalChain([t, 'optionalAccess', _292 => _292.soon])||"Binnenkort",val:soon.length,color:soon.length?"#FFD54F":C.greenLight,
             bg:soon.length?"rgba(212,150,14,0.2)":"rgba(255,255,255,0.08)",
             icon:"⏳"},
            {label:_optionalChain([t, 'optionalAccess', _293 => _293.avgHealth])||"Gem. gezondheid",
          ...
          React.createElement('div', { className: "pca-fade", style: {background:`linear-gradient(135deg, ${C.terracottaLight}, #F5D0C4)`,
            ...
            , urgent.map(p=>(
              React.createElement('div', { key: p.id, onClick: ()=>onSelectPlant(p),
                ...
                , React.createElement('strong', null, p.name)
                , React.createElement('span', { style: {color:C.muted,fontSize:11},}, "(", _optionalChain([SPECIES_DB, 'access', _294 => _294[p.species], 'optionalAccess', _295 => _295.commonName])||p.species, ")")
            ...
            , ["Alle",...sections.map(s=>s.name)].map(name=>{
              ...
              return (
                React.createElement('button', { key: name, onClick: ()=>setFilterSection(name),
                  style: {padding:"5px 12px",borderRadius:20,border:`1px solid ${active?(_optionalChain([s, 'optionalAccess', _296 => _296.color])||C.green):C.border}`,
                    background:active?(_optionalChain([s, 'optionalAccess', _297 => _297.color])||C.green)+"22":"none",
                    color:active?(_optionalChain([s, 'optionalAccess', _298 => _298.color])||C.green):C.muted,
        ...
        , showGrouped && groupedPlants ? (
              ...
              , sPlants.map((plant, idx) => (
                React.createElement('div', { key: plant.id, style: {animationDelay:`${idx*40}ms`},}
                  , React.createElement(PlantCard, { plant: plant, logs: logs, sections: sections,
                    ...
                    isSelected: _optionalChain([selectedPlants, 'optionalAccess', _299 => _299.includes, 'call', _300 => _300(plant.id)]), bulkMode: bulkMode,})
                )
              ))
            )
          ))
          ...
          displayed.length === 0 ? (
                ...
                React.createElement('div', { key: plant.id, style: {animationDelay:`${idx*40}ms`},}
                  , React.createElement(PlantCard, { plant: plant, logs: logs, sections: sections,
                    onSelect: onSelectPlant, onQuickWater: onQuickWater,
                    isSelected: _optionalChain([selectedPlants, 'optionalAccess', _301 => _301.includes, 'call', _302 => _302(plant.id)]), bulkMode: bulkMode,})
                )
              ))
        )
      )
...
function PlantCareApp() {
  ...
  useEffect(() => {
    ...
    const missingAddedLogs = plants.filter(plant => {
      ...
      const hasAddedLog = logs.some(l =>
        l.plantId === plant.id &&
        l.auto === true &&
        _optionalChain([l, 'access', _303 => _303.notes, 'optionalAccess', _304 => _304.includes, 'call', _305 => _305("toegevoegd aan de collectie")])
      );
      return !hasAddedLog;
    });
  ...
  const quickWater = useCallback((plant) => {
    const season = getCurrentSeason();
    const care = _optionalChain([SPECIES_DB, 'access', _306 => _306[plant.species], 'optionalAccess', _307 => _307.care, 'access', _308 => _308[season]]);
    const interval = _nullishCoalesce(_nullishCoalesce(_optionalChain([plant, 'access', _309 => _309.customCare, 'optionalAccess', _310 => _310[`water_${season}`]]), () => ( _optionalChain([care, 'optionalAccess', _311 => _311.water]))), () => ( 7));
    const today = new Date().toISOString().split("T")[0];
  ...
  const deleteSection=useCallback(id=>{
    ...
    setPlants(prev=>prev.map(p=>p.section===_optionalChain([sections, 'access', _312 => _312.find, 'call', _313 => _313(s=>s.id===id), 'optionalAccess', _314 => _314.name])?{...p,section:""}:p));
  },[sections]);
  const renameSection=useCallback((id,name,color)=>setSections(prev=>prev.map(s=>s.id===id?{...s,name,color:color||s.color}:s)),[]);

  if(selected)return(React.createElement(PlantDetail, { plant: selected, logs: logs, sections: activeRooms, onAddSection: addSection, onBack: ()=>setSelectedId(null), onUpdatePlant: updatePlant, onAddLog: addLog, onDeleteLog: deleteLog, onUpdateLog: updateLog, onDelete: deletePlant, t: t,}));
  return(React.createElement(React.Fragment, null
    ...
    , showSettings
      ? React.createElement(SettingsScreen, {
          ...
          estates: estates, activeEstateId: activeEstateId,
          onAddEstate: (name,icon)=>{const e={id:uid(),name,icon};setEstates(prev=>[...prev,e]);setActiveEstateId(e.id);},
          onDeleteEstate: id=>{setEstates(prev=>prev.filter(e=>e.id!==id));if(activeEstateId===id)setActiveEstateId(_optionalChain([estates, 'access', _315 => _315.find, 'call', _316 => _316(e=>e.id!==id), 'optionalAccess', _317 => _317.id])||"");},
      ...
      : React.createElement(Dashboard, {
          plants: plants, logs: logs, sections: activeRooms,
